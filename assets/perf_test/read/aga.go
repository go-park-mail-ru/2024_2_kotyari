...
func (a *Attacker) getProductDetails(metrics *Metrics) error {
	handle := a.viper.GetStringMapString(handlers)
	url := a.url + handle[getOrdersHandler]
	...
	if resp.StatusCode != http.StatusOK {
		err = fmt.Errorf("unexpected status code %d when fetching product details: %s", resp.StatusCode, string(bodyBytes))
		metrics.RecordRequest(resp.StatusCode, bytesIn, bytesOut, latency, err)
		return err
	}
...
func runLoadReadTest(attackers []*Attacker, products []int, concurrency int, iterations int, metrics *Metrics, rps int) {
		...
		for _, product := range products {
			for _, attacker := range attackers {
				...
				go func(a *Attacker, product int, iteration int) {
					...
					if err := a.getProductDetails(metrics); err != nil {
						// Можно раскомментировать для детального логирования ошибок
						// log.Printf("Iteration %d: Error fetching product details for product %d for user %s: %v", iteration+1, product, a.userID, err)
						return
					}

					// Можно добавить логирование успешных операций
					// log.Printf("Iteration %d: Successfully fetched product details for product %d for user %s", iteration, product, a.userID)
				}(attacker, product, i)
			}
		}
	}
...
func main() {
	...
	log.Printf("Starting Load Read Test with %d users, %d concurrency, %d iterations and %d RPS", numUsers, concurrency, iterations, rps)
	runLoadReadTest(attackers, products, concurrency, iterations, metrics, rps)
}
