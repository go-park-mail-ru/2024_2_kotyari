...
CREATE TABLE IF NOT EXISTS "user" (
	...
	"time_to_block" integer NOT NULL DEFAULT '-1' CHECK (time_to_block >= -1),
...
CREATE TABLE IF NOT EXISTS "cart" (
	...
	PRIMARY KEY ("user", "product_id"),  -- Комбинированный первичный ключ, чтобы каждая пара была уникальной
	FOREIGN KEY ("user") REFERENCES "user"("id"),
	FOREIGN KEY ("product_id") REFERENCES "product"("id")
);
...
-- Таблица для хранения выбранных значений опций для конкретного продукта в заказе
